...
public class CustomBuilderEditorWindow : EditorWindow 
{
    ...
    void CreateRuleObjects()
    {
        if (previewObject == null)
        {
            ...
            previewObject.transform.position = new UnityEngine.Vector3(0, 0, -500);
            ...
            for (int y = -1; y <= 1; y++)
                for (int x = -1; x <= 1; x++)
                    for (int z = -1; z <= 1; z++)
                    {
                        ...
                        var material = AssetDatabase.LoadAssetAtPath("Assets/Art/EditorMaterials/CustomBuilderPreview.mat", typeof(Material)) as Material;
                        if (material != null)
                            renderer.sharedMaterial = new Material(material);

                        OffsetCondition.Offset condition = OffsetCondition.GetFromVector(new Vector3i(x, y, z));
    ...
    // show the grid of neighbors to add conditions for, defaulting to all null (dont care)
    void ShowNeighborGrids()
    {
        ...
        EditorGUILayout.BeginVertical(GUILayout.MaxWidth(400));
        {
            ...
            topLayer.SetActive(ShowLayer(Vector3i.Up, "Top", ref topLayerSelection, topLayer.activeSelf));
            middleLayer.SetActive(ShowLayer(Vector3i.Zero, "Middle", ref midLayerSelection, middleLayer.activeSelf));
            bottomLayer.SetActive(ShowLayer(Vector3i.Down, "Bottom", ref bottomLayerSelection, bottomLayer.activeSelf));
            ...
            selectedUsageCase.decorativeBuilder = (CustomBuilder)EditorGUILayout.ObjectField("Decoration Builder", selectedUsageCase.decorativeBuilder, typeof(CustomBuilder), false);
        }
        EditorGUILayout.EndVertical();
    }

    bool ShowLayer(Vector3i layerOffset, string name, ref int selection, bool enabled)
    {
        string[] layers = new string[9];
        Vector3i[] offsets = new Vector3i[9];
        ...
        for (int z = 1; z >= -1; z--)
            for (int x = -1; x <= 1; x++)
            {
                var pos = new Vector3i(x, 0, z);

                Vector3i layerPos = layerOffset + pos;
                offsets[i] = layerPos;

                if (layerPos == Vector3i.Zero)
                    layers[i] = "+";
                else
                {
        ...
        int newSelection = GUILayout.SelectionGrid(selection, layers, 3);
